...
class Trivia {
	...
	{	$respuesta = $this->db->enviarQuery("select id_tr , texto_tr , vig_ini_tr, vig_fin_tr, perfil_tr from trivia_tr order by vig_fin_tr ");
		if (!$respuesta and $this->db->error!=''){echo $this->db->error; return false;}else{if (!$respuesta){return false;}else {return $respuesta;}}
	}
	...
	{	
		...
		$respuesta = $this->db->enviarQuery("select * from trivia_tr tr inner join pregunta_pr pr on tr.id_tr = pr.id_trivia_pr where pr.id_pr not in (select ask_re from  respuesta_re where usuario_re = $_user) and  CURRENT_DATE BETWEEN  vig_ini_tr and vig_fin_tr and tr.perfil_tr like '%".$_perfil."%' LIMIT 1");
		if (!$respuesta and $this->db->error!=''){echo $this->db->error;return false;}else{if (!$respuesta){	return false;}	else {	return $respuesta;}}
	}
